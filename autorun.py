...
<dict>
    <key>Label</key>
    <string>com.sgfc.musicbar</string>
    <key>ProgramArguments</key>
    <array>
        <string>/usr/local/bin/musicbar</string>
    </array>
    ...
    if sys.argv[len(sys.argv) - 1].lower() == 'enable':
        ...
        print(
            "Running command 'launchctl load -w " + plistNewPath + "' to enable musicbar on login.\n")
        subprocess.call(["launchctl", "load", "-w", plistNewPath])

        print(
            "If 'service already loaded' then run the command:\n\nlaunchctl start " + plistLabel + "\n")
